# !/usr/bin/env python3
"""
to_py_Original.py - 运行xxdz CzeData文件的弹窗播放器模板
"""
...
# ============================================================================
CZEDATA_CONTENT = None  # 请将您的CzeData JSON内容粘贴到这里，替换None
# ============================================================================
# 结束 CZEDATA 内容
# ============================================================================
...
class WindowAnimationExecutor:
        ...
        try:
            ...
            try:
                ...
                for anim in animations:
                    total_duration += anim.get('duration', 1000)
                animation_start_time = time.time()
                animation_end_time = animation_start_time + (total_duration / 1000.0)
                print(f"动画总时长: {total_duration}ms, 开始时间: {animation_start_time}, 结束时间: {animation_end_time}")
                ...
                if animations and 'target_type' in animations[0]:
                    ...
                    for target_key, target_data in target_animations.items():
                        self.execute_animations_for_target(
                            target_data['target_code'],
                            target_data['target_name'],
                            target_data['animations']
                        )
                        # 记录待执行任务（不记录时间）
                        target_tasks.append({
                            'target_code': target_data['target_code'],
                            'target_name': target_data['target_name'],
                            'animations': target_data['animations']
                        })
                else:
                ...
                # ===== ：保存待执行动画任务（不保存时间） =====
                if target_tasks:
                    with self.command_executor.animation_lock:
                        # 添加新任务（不进行过期检查）
                        self.command_executor.pending_animations.extend(target_tasks)
                    print(f"已记录 {len(target_tasks)} 个动画任务，供后续窗口执行")
    ...
    def execute_single_animation(self, window, anim):
        ...
        try:
            ...
            # 执行动画（不传递时间参数）
            self.animate_window_movement(
                window, current_x, current_y,
    ...
    def animate_window_movement(self, window, start_x, start_y, target_x, target_y, duration, easing):
        ...
        def update_position(step):
            ...
            try:
                ...
                # 重要修复：对于镂空窗口，geometry设置时尺寸和位置之间必须有空格
                # 格式必须是 "宽度x高度+X+Y" 中间没有空格
                if window.winfo_exists():
                    # 确保坐标是整数
                    geometry_str = f"{current_width}x{current_height}+{current_x}+{current_y}"
...
class GlobalCommandExecutor:
    ...
    def __init__(self, player):
        ...
        # ===== ：存储待执行的动画任务 =====
        self.pending_animations = []  # 格式: [{'target_code': '', 'target_name': '', 'animations': [], 'start_time': 0, 'end_time': 0}, ...]
        self.animation_lock = threading.Lock()  # 动画任务锁
...
class CzePlayer:
    def __init__(self):
        ...
        # 预计算缓存系统
        self.image_cache = {}  # 图片缓存 { (image_path, hollow_enabled, hollow_color, size_option, custom_width, custom_height): (img, width, height) }
        self.cache_lock = threading.Lock()  # 缓存锁
        self.preload_completed = False  # 预加载完成标志
    ...
    def load_czedata(self, filepath=None):
        """加载CzeData文件 - 支持从文件或内置变量加载"""
        try:
            # 从文件加载（兼容原有模式）
            if filepath and os.path.exists(filepath):
                ...
                # 从内置变量 CZEDATA_CONTENT 加载
                global CZEDATA_CONTENT
                if CZEDATA_CONTENT is None:
                    print("错误: CZEDATA_CONTENT 未设置，请将您的CzeData JSON内容粘贴到该变量中")
                    return False
                data = CZEDATA_CONTENT
                print("从内置变量 CZEDATA_CONTENT 加载数据")
    ...
    def preload_images(self):
        ...
        for note in self.notes:
            ...
            # 创建缓存键
            cache_key = (actual_path, hollow_enabled, hollow_color, size_option, custom_width, custom_height)

            # 如果已经在缓存中，跳过
            if cache_key in self.image_cache:
                continue

            preload_tasks.append((cache_key, actual_path, hollow_enabled, hollow_color,
                                  size_option, custom_width, custom_height))
        ...
        print(f"需要预加载 {len(preload_tasks)} 张图片")

        def process_image(task):
            """处理单张图片"""
            cache_key, actual_path, hollow_enabled, hollow_color, size_option, custom_width, custom_height = task
            ...
            try:
                ...
                if hollow_enabled:
                    if hollow_color.startswith('#'):
                        target_rgb = tuple(int(hollow_color.lstrip('#')[i:i + 2], 16) for i in (0, 2, 4))
                    else:
                    ...
                    for pixel in data:
                        if len(pixel) >= 4:
                            ...
                            if (abs(r - target_rgb[0]) <= color_tolerance and
                                    abs(g - target_rgb[1]) <= color_tolerance and
                                    abs(b - target_rgb[2]) <= color_tolerance):
                ...
                if size_option == 'image_size':
                    ...
                    print(f"预加载等比缩放: 原始 {orig_width}x{orig_height} -> 目标 {img_width}x{img_height}")

                # 存入缓存
                with self.cache_lock:
                    self.image_cache[cache_key] = (img, img_width, img_height)

                print(f"   预加载完成: {os.path.basename(actual_path)} ({img_width}x{img_height})")

            except Exception as e:
                print(f"   预加载失败 {os.path.basename(actual_path)}: {e}")

        # 使用线程池预加载
        threads = []
        for task in preload_tasks:
            thread = threading.Thread(target=process_image, args=(task,), daemon=True)
            thread.start()
    ...
    def calculate_aligned_position_with_actual_size(self, position_info, actual_width, actual_height):
        """
        根据实际窗口尺寸重新计算对齐位置
        注意：actual_width, actual_height 是等比缩放后的实际窗口尺寸
        """
        coords = position_info.get('align_coords', {})
        coord_type = coords.get('coord_type', 'abs')
        direction = coords.get('direction', '↖')
    ...
    def calculate_link_position(self, position_info):
        ...
        try:
            ...
            # 优先使用窗口的目标位置（最终位置），避免动画中的临时坐标影响链接计算
            if hasattr(base_window, '_target_x') and hasattr(base_window, '_target_y'):
                base_x = base_window._target_x
                base_y = base_window._target_y
                base_width = base_window._target_width
                base_height = base_window._target_height
                print(f"  使用目标位置作为基准: ({base_x}, {base_y}), 目标尺寸: {base_width}x{base_height}")
            else:
                ...
                print(f"  使用实时位置作为基准: ({base_x}, {base_y}), 实时尺寸: {base_width}x{base_height}")

            print(f"  基准窗口位置: ({base_x}, {base_y}), 尺寸: {base_width}x{base_height}")

            # 注意：新窗口的尺寸在计算时可能未知，这里使用默认值，后续会在 create_window_now 中根据实际尺寸调整
            window_width = 350
            window_height = 100
            ...
            # 边界保护使用默认尺寸，但实际窗口可能不同，会在 create_window_now 中重新调整
            x = max(0, min(x, self.screen_width - window_width))
            y = max(0, min(y, self.screen_height - window_height))
    ...
    def create_window_now(self, note):
        ...
        try:
            ...
            if window_type == '图片框':
                ...
                print(f"窗口实际尺寸（等比缩放后）: {actual_width}x{actual_height}")

                # 如果是对齐位置，使用缩放后的实际尺寸重新计算
                if 'window_position' in cze and cze['window_position'].get('type') == 'align':
                    ...
                    print(f"图片框对齐位置重新计算（使用缩放后实际尺寸 {actual_width}x{actual_height}）: {position}")
            ...
            popup._target_x = x
            popup._target_y = y
            popup._target_width = actual_width
            popup._target_height = actual_height
            ...
            # ===== ：检查并执行待处理的动画 =====
            def check_pending_animations():
                try:
                    with self.command_executor.animation_lock:
                        # 复制待执行动画列表，避免迭代时修改
                        pending_animations = self.command_executor.pending_animations.copy()

                    # 检查每个待执行动画
                    for anim_task in pending_animations:
                        target_code = anim_task['target_code']
                        target_name = anim_task['target_name']
                        animations = anim_task['animations']

                        # 检查窗口是否匹配目标
                        is_match = False
                        if target_code == 'group' and window_group == target_name:
                            is_match = True
                        elif target_code == 'class' and window_class == target_name:
                            is_match = True

                        if is_match:
                            print(f"新窗口匹配待执行动画: {target_code}:{target_name}")
                            # 立即执行动画，不检查时间
                            self.command_executor.animation_executor.execute_animations_for_window(
                                popup, animations
                            )
                            break  # 每个窗口只执行一次匹配的动画
                except Exception as e:
                    print(f"检查待执行动画失败: {e}")

            # 延迟一点执行，确保窗口完全创建
            popup.after(100, check_pending_animations)
            ...
            if window_type == '图片框':
                ...
                popup.after(100, lambda: bind_children(popup))
    ...
    def create_image_window(self, popup, cze, title, window_life_ms, window_class, window_group, position=None):
        """创建图片框窗口（支持完全镂空透明效果，无窗口边框）- 优化版：进入/退出动画独立开关，仅缩放"""
        # 获取镂空设置
        hollow_enabled = cze.get('hollow_enabled', False)
        hollow_animation = cze.get('hollow_animation', False)          # 进入动画
        hollow_unload_animation = cze.get('hollow_unload_animation', False)  # 退出动画
        hollow_color = cze.get('hollow_color', '#FFFFFF')
        normal_animation = cze.get('normal_animation', False)          # 普通图片框进入动画（仅作兼容）
        image_path = cze.get('image_path', '')
        size_option = cze.get('size_option', 'image_size')
        custom_width = cze.get('custom_width', 400)
        custom_height = cze.get('custom_height', 300)
        ...
        # 定义内部函数（必须先定义，再使用）
        def calculate_final_position(img_width, img_height):
            """
            计算最终位置
            注意：传入的 img_width, img_height 已经是等比缩放后的实际尺寸
            """
            if position:
                # 如果是对齐位置，使用实际尺寸重新计算
                if hollow_enabled and 'window_position' in cze and cze['window_position'].get('type') == 'align':
                    ...
                    if hasattr(self, 'calculate_aligned_position_with_actual_size'):
                        # 使用缩放后的实际尺寸计算对齐位置
                        return self.calculate_aligned_position_with_actual_size(
                            position_info, img_width, img_height)
                # 其他位置类型，直接使用预计算的位置（已在 create_window_now 中计算好）
                return position
            # 默认居中显示，使用缩放后的实际尺寸
            return ((self.screen_width - img_width) // 2, (self.screen_height - img_height) // 2)

        def enter_animation(img_width, img_height, final_x, final_y):
            """进入动画：从 60% 缩放到 100%，中心固定"""
            print(" 进入动画开始")
            duration = 150
            steps = 10
            delay = duration // steps
            ...
            # 最终窗口中心
            center_x = final_x + img_width // 2
            center_y = final_y + img_height // 2

            # 初始缩放尺寸
            start_width = int(img_width * start_scale)
            start_height = int(img_height * start_scale)
            start_x = center_x - start_width // 2
            start_y = center_y - start_height // 2
            popup.geometry(f"{start_width}x{start_height}+{start_x}+{start_y}")
            popup.update_idletasks()
            ...
            def run(step=0):
                ...
                progress = step / steps
                eased = 1 - (1 - progress) * (1 - progress)  # ease-out
                current_scale = start_scale + (end_scale - start_scale) * eased
                w = int(img_width * current_scale)
                h = int(img_height * current_scale)
                x = center_x - w // 2
                y = center_y - h // 2
                ...
                popup.after(delay, lambda: run(step + 1))

            run(0)

        def exit_animation(img_width, img_height, final_x, final_y):
            """退出动画：从 100% 缩放到 60%，中心固定，完成后关闭窗口"""
            print(" 退出动画开始")  # 调试输出
            duration = 150
            steps = 10
            delay = duration // steps
            ...
            # 当前窗口状态
            current_w = popup.winfo_width()
            current_h = popup.winfo_height()
            ...
            target_w = int(img_width * 0.6)
            target_h = int(img_height * 0.6)
            target_x = center_x - target_w // 2
            target_y = center_y - target_h // 2
            ...
            def run(step=0):
                ...
                w = int(current_w + (target_w - current_w) * eased)
                h = int(current_h + (target_h - current_h) * eased)
                x = int(current_x + (target_x - current_x) * eased)
                y = int(current_y + (target_y - current_y) * eased)
        ...
        def close_window():
            """关闭窗口（带动画）"""
            if hasattr(popup, '_play_exit_animation') and popup._play_exit_animation:
                popup._play_exit_animation()
            else:
                cleanup_and_close()

        # 创建缓存键
        cache_key = (actual_image_path, hollow_enabled, hollow_color, size_option, custom_width, custom_height)

        # === 极致优化：缓存命中时直接显示 ===
        with self.cache_lock:
            if cache_key in self.image_cache:
                img, img_width, img_height = self.image_cache[cache_key]
                print(f" 极速模式: 使用缓存图片 {os.path.basename(actual_image_path)}")

                # 立即设置窗口背景（无延迟）
                if hollow_enabled and sys.platform == 'win32':
                    try:
                        popup.configure(bg=hollow_color)
                        popup.wm_attributes('-transparentcolor', hollow_color)
                    except:
                        popup.configure(bg='white')
                else:
                    popup.configure(bg='white')

                # 立即创建 PhotoImage 和标签（不显示loading）
                try:
                    photo = ImageTk.PhotoImage(img, master=popup)

                    if hollow_enabled:
                        img_label = tk.Label(popup, image=photo, bg=hollow_color, bd=0, highlightthickness=0)
                    else:
                        img_label = tk.Label(popup, image=photo, bg='white', bd=0, highlightthickness=0)

                    img_label.image = photo
                    img_label.pack(fill='both', expand=True)
                    img_label.bind('<Button-3>', lambda e: close_window())

                    # 计算最终位置
                    final_x, final_y = calculate_final_position(img_width, img_height)

                    # 处理进入动画（如果有）
                    if (hollow_enabled and hollow_animation) or (not hollow_enabled and normal_animation):
                        enter_animation(img_width, img_height, final_x, final_y)
                    else:
                        popup.geometry(f"{img_width}x{img_height}+{final_x}+{final_y}")

                    # 设置退出动画（仅镂空且启用退出动画时）
                    if hollow_enabled and hollow_unload_animation:
                        popup._play_exit_animation = lambda: exit_animation(img_width, img_height, final_x, final_y)
                        popup._has_animation = True
                        print(" 退出动画已注册")  # 调试输出

                    popup.update_idletasks()
                    print(f" 图片框极速创建成功: {img_width}x{img_height}")

                except Exception as e:
                    print(f"显示图片失败: {e}")
                    error_label = tk.Label(popup, text=f"无法加载图片", fg='red')
                    error_label.pack(expand=True)
                    error_label.bind('<Button-1>', lambda e: cleanup_and_close())
        ...
        if hollow_enabled and sys.platform == 'win32':
            try:
                popup.configure(bg=hollow_color)
                popup.wm_attributes('-transparentcolor', hollow_color)
                print(f"镂空模式: Windows 透明窗口设置成功，透明色: {hollow_color}")
            except Exception as e:
        ...
        # 显示加载提示
        loading_label = tk.Label(popup, text="加载中...", font=('Segoe UI', 10))
        ...
        def process_image_thread():
            try:
                ...
                if hollow_enabled:
                    ...
                    img.putdata(new_data)
                    print(f"镂空处理完成: {hollow_count}/{total_pixels} ({hollow_count / total_pixels * 100:.1f}%)")

                # 调整图片大小 - 支持等比缩放
                if size_option == 'image_size':
                    # 按图片原始尺寸显示，但如果超过最大限制则等比缩小
                    img_width, img_height = img.size
                    max_width = 600
                    max_height = 400
                    if img_width > max_width or img_height > max_height:
                    ...
                    img_height = target_height
                    img = img.resize((img_width, img_height), Image.Resampling.LANCZOS)
                    print(f"等比缩放: 原始 {orig_width}x{orig_height} -> 目标 {img_width}x{img_height}")

                # 存入缓存
                with self.cache_lock:
                    self.image_cache[cache_key] = (img, img_width, img_height)
                    print(f" 图片已加入缓存: {os.path.basename(actual_image_path)}")

                # 在主线程中更新UI
                popup.after(0, lambda: display_image(img, img_width, img_height))

            except Exception as e:
                print(f"图片处理失败: {e}")
                import traceback
                traceback.print_exc()
                popup.after(0, lambda: show_error())

        def display_image(img, img_width, img_height):
            """在主线程中显示图片"""
            try:
                ...
                popup.update_idletasks()
                print(f"图片框创建成功: {img_width}x{img_height}")
    ...
    def create_tray_icon(self):
        ...
        if not TRAY_AVAILABLE:
            print("托盘图标库 pystray 未安装，无法创建托盘图标")
            return False

        if sys.platform != 'win32':
            print("非Windows系统，不支持托盘图标")
            return False
        ...
        try:
            ...
            for icon_path in icon_paths_to_try:
                if os.path.exists(icon_path):
                    ...
                    print(f"找到托盘图标文件: {icon_path}")
                    try:
                        icon_image = Image.open(icon_path)
                        icon_image = icon_image.resize((64, 64), Image.Resampling.LANCZOS)
                        print(f"成功加载托盘图标: {icon_path}")
                        break
                    except Exception as e:
                        print(f"加载托盘图标文件 {icon_path} 失败: {e}")
                        icon_image = None  # 重置，尝试下一个

            # 如果找不到图标或加载失败，创建一个简单的图标
            if icon_image is None:
                print("未找到有效图标文件，使用默认蓝色图标")
            ...
            def run_tray():
                try:
                    print("托盘图标线程启动")
                    self.tray_icon.run()
                    print("托盘图标线程结束")  # 正常情况下不会执行到这里
                except Exception as e:
                    print(f"托盘图标运行出错: {e}")

            threading.Thread(target=run_tray, daemon=True).start()
            print("托盘图标线程已启动，请检查系统托盘区域（可能需要展开隐藏图标噢）")
            return True
    ...
    def run(self):
        """运行播放器 - 仅从内置变量 CZEDATA_CONTENT 加载数据"""
        self.get_screen_resolution()

        # 仅从内置变量加载数据
        print("从内置变量 CZEDATA_CONTENT 加载数据...")
        success = self.load_czedata()

        if success:
            # 预加载图片
            preload_thread = threading.Thread(target=self.preload_images, daemon=True)
            ...
            print("加载数据失败！")
            print("请确保 CZEDATA_CONTENT 变量已正确设置为您的CzeData JSON内容")
            print("当前 CZEDATA_CONTENT 的值: None")
            input("按回车键退出...")
